...
export default class Login extends Vue {
    ...
    private beforeUpdate() {
        this.$logger.log('生命周期：beforeUpdate');
    }
    // 由于数据更改导致的虚拟 DOM 重新渲染和打补丁，在这之后会调用该钩子。
    private updated() {
        this.$logger.log('生命周期：updated');
    }
    // 被 keep-alive 缓存的组件激活时调用。
    private activated() {
        this.$logger.log('生命周期：activated');
    }
    // 被 keep-alive 缓存的组件停用时调用。
    private deactivated() {
        this.$logger.log('生命周期：deactivated');
    }
    // 实例销毁之前调用。在这一步，实例仍然完全可用。
    private beforeDestroy() {
        this.$logger.log('生命周期：beforeDestroy');
    }
    // 生命周期 实例销毁后调用。该钩子被调用后，对应 Vue 实例的所有指令都被解绑，所有的事件监听器被移除，所有的子实例也都被销毁。
    private destroyed() {
        this.$logger.log('生命周期：destroyed');
    }
    // 当捕获一个来自子孙组件的错误时被调用。此钩子会收到三个参数：错误对象、发生错误的组件实例以及一个包含错误来源信息的字符串。此钩子可以返回 false 以阻止该错误继续向上传播。
    private errorCaptured() {
        this.$logger.log('生命周期：errorCaptured');
    }
    // 窗口改变大小
    ...
    private login() {
        ...
        LoginModule.login(this.userInfo).then(res => {
            ...
            if (code === 200) {
                ...
                this.$router.push(RouterPaths.main);
            } else {
                this.$message.error(message);
            }
        });
        LoginModule.testFun(456).then(res => {
            this.$logger.log('使用 @MutationAction 时，res 没有返回值，也不需要返回值，因为已经维护到 store中：')(res);
        });
    }
}
